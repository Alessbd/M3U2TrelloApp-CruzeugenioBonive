...
const crearTarea = (tarea) => {
  ...
  divTarjeta.innerHTML = `<h5 class="card-title text-card-css">Título: ${tarea.title} </h5><h6 class="card-subtitle mb-2 ">Descripción: ${tarea.details}</h6><h6 class="card-subtitle mb-2 mt-1 text-muted">Responsable: ${tarea.person}</h6><p class="card-text text-weight">Plazo: ${dateFormat(tarea.deadline)} </p><p class="card-text text-weight">Creación: ${dateFormat(tarea.created)} </p>`
